...
public class LoginController {
    ...
    public boolean login(@RequestBody LoginDto loginDto, HttpServletRequest request){
        ...
        System.out.println(request.getHeader("User-Agent")); // 통신을 요청한 클라이언트(사용자)의 브라우저정보 확인,
        //*** 세션 정보 가져오기, 세션이란? 톰캣 서버내 저장소(key-value / MAP 컬렉션 사용)
        HttpSession session = request.getSession();
        System.out.println(session.getId());                  //세션의 식별 번호: 브라우저 마다, pc마다 다르게 할당 된다.
        ...
        // *** 세션 속성을 추가하기, 속성이란? 키와 값으로 구성된 메모리 공간
        session.setAttribute("loginMno",3); // .setAttribute("속성명",속성값);
        //loginMno 라는 속성명으로 3이라는 데이터 저장 하겠다는 뜻
        return true;
    }//func end
    // 2. 로그인된 정보 확인 : 현재 세선에 저장된 회원정보 확인
    @GetMapping("/info")
    public boolean info(HttpServletRequest request){
        //1. 요청 서블릿에서 세션정보 꺼내기
        HttpSession session = request.getSession();
        //2. 세션정보내 원하는 속성명 값 꺼내기
