...
class SampleController{
    SampleDao1 sampleDao1 = new SampleDao1(); // 다른 클래스의 메소드 호출 방법
    public void  method2(){
        sampleDao1.method1();
    } //func end
} //class end

// =======[방법2] 해당 인스턴스 미리(new) 생성하여 메소드 호출 = 싱글톤
class SampleDao2{
...
class SampleController2{ // 다른 클래스의 메소드 호출 방법
...
// =======[방법3] IOC 와 DI
@Component // + 스프링 컨테이너에 빈(객체/인스턴스) 등록, 스프링 실행 시 *자동*으로 인스턴스 생성
class  SampleDao3{
    void method1(){}
}//class end
...
class SampleController4{
    private final SampleDao3 sample3; // final(수정불가기능키워드,초기값 필수) 사용함으로 불변성(안전)보장

    @Autowired // 생성자에 빈 주입 함으로써
    public SampleController4( SampleDao3 sampleDao3 ){this.sample3=sampleDao3;}
