...
@Service // 해당 클래스를 스프링 컨테이너 빈 등록
public class FileService {

    //[*] 업로드 경로,
    // 1) 프로젝트 src   : 개발자가 코드를 작성하는 폴더
    // 2) 프로젝트 build : 서버실행했을때 컴파일된 코드 즉] 서버(24시간)의 갖는 실행된 폴더

    // 1. 현재 프로젝트의 최상위 디렉토리(폴더) 경로 찾기
    private String baseDir = System.getProperty("user.dir");

    // 2. 방법2 처럼 개발자 폴더가 아닌 실행된 서버의 폴더로 업로드경로 지정하기, *개발환경*에 따라 달라진다.
    private String uploadPath = baseDir + "/build/resources/main/static/upload/";

    //[1] 파일 업로드 : 스프링에서는 MultipartFile 인터페이스 차원 (대용량 바이트 조작)
    public String fileUpload(MultipartFile multipartFile){
        //1. 업로드한 파일명이 중복일때, 다른 파일 이지만 파일명은 같을 수 있다.
        // 방법1) 파일명 앞에 PK 번호를 붙인다.
        ...
        String uuid = UUID.randomUUID().toString();
            ...
            // 이유 : uuid  와 파일명 구분을 _언더바 사용하기 위한 내부적 규칙
        String fileName = uuid +"_"+multipartFile.getOriginalFilename().replaceAll("_","-");
            // .replaceAll("기존문자"+"새로운 문자"): 기본문자를 새로운문자로 변환 메소드

        //2. 업로드 경로 와 파일명 합치기
        String uploadFilePath = uploadPath + fileName;

        //3. 만약에 업로드한 경로가 upload 폴더가 존재 하지 않으면 폴더 생성
        File pathFile = new File(uploadPath); //File 클래스: 다양한 파일/폴더 함수를 제공
        if(!pathFile.exists()) {//.exists() : 지정한 경로가 존재 하면 true 존재 하지않으면 false;
        ...
        try {
            multipartFile.transferTo(uploadFile); // multipartFile(업로드된 파일) //일반예외발생
    ...
    public void fileDownload(String fileName, HttpServletResponse response){
        ...
        if(!file.exists()){return;}//if end
        //3. 업로드 할 파일을 자바(바이트)로 가져오기(파일 읽기)
        try{

            //3-1 파일 입력스트림 객체 생성
            FileInputStream fin = new FileInputStream(downloadPath);
            //3-2 파일 용량 만큼 배열 생성, 읽어온 바이트들을 저장할 배변변수 선언
            long fileSize = file.length(); // 파일 용량/바이트 수
            byte[] bytes = new byte[(int)fileSize]; // 파일 용량만큼 바이트 배열 변수 선언
            //3-3 파일 입력스트림 객체로 파일을 읽어와서 배열에 저장
            fin.read(bytes);
            // 3-4 안전하게 스트림 닫기, 스트림이란? 바이트가 다니는 (이동)통신 경로
            fin.close(); //필수는 아니지만 대용량에서는 안전하게 스트림 객체 를 직접 닫기 권장!

            //************** 다운로드 형식 지정 : 브라우저 마다 상이 ******************//
            //**1. 파일의 uuid 제거, split("기준문자"):문자열 내 기준문자 기준으로 문자열 자르기
            String oldFilename= fileName.split("_")[1]; //UUID-파일명, -하이픈 기준으로 잘라서 2번째 문자열 뜻
            //**2. 응답 형식 지정하기, 다운로드 형식과 다운로드 파일 지정, URL은 한글 지원하지 않는다. URLEncoder.encode(파일명,"UTF-8");
            response.setHeader("Content-Disposition", "attachment;filename="+ URLEncoder.encode(oldFilename,"UTF-8"));

        //4. 업로드할 파일을 사용자에게 (repones 객체 이용한)응답하기.
    ...
    public boolean fileDelete(String fileName){
        ...
        if(file.exists()){file.delete(); return true;}//if end //.delete():지정한 경로상의 파일 삭제
        //3. 아니면 false;
        return false;
    }//func end
